...
use crate::cpalaudio;
use crate::player_engine::{PlayerActions, PlayerStatus, Playing};
use crate::{
    cpalaudio::{AudioOutput, CpalAudioOutput},
    url_source_buff::UrlSourceBuf,
};

pub struct PlayerEngineAsync {
    ...
    pub fn start(&mut self) -> Result<i32> {
        ...
        let result = loop {
            ...
            if let Some(ref a) = action {
                match a {
                    PlayerActions::Close => {
                        self.notify_toggle_play.notify_one();
                        break Ok(0) //todo!(),
                    }
...
impl Drop for PlayerEngineAsync {
    fn drop(&mut self) {
